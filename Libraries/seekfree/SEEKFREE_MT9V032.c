...
volatile IMG_STATUS mt9v032_image_flag;
...
{
   //场中断需要判断是场结束还是场开始
    if(mt9v032_image_flag == IMG_START)                   //需要开始采集图像
    {
        mt9v032_image_flag = IMG_GATHER;                  //标记图像采集中
        disable_irq(PORTC_IRQn);
        
        PORTC_FLAG_CLR(C18);
       // PORTC_ISFR = 1 <<  PT18;            //清空PCLK标志位
        DMA0->TCD[CAMERA_DMA_CH].DADDR = (uint32)image;
        DMA_EN(CAMERA_DMA_CH);                  //使能通道CHn 硬件请求
    }
    else                                        //图像采集错误
    {
        disable_irq(PORTC_IRQn);                        //关闭PTC的中断
        mt9v032_image_flag = IMG_FAIL;                    //标记图像采集失败
    }

//	MT9V032_INTERRUPT_ISFR = 1 << MT9V032_VSYNC_CHANNEL;
  //  if(!mt9v032_finish_flag)    DMA_EN(DMA_CH0);                //使能DMA
}
...
/*!
 *  @brief      总钻风采集图像（采集到的数据存储在image[][]上）
 *  @since      v5.0
 */
void mt9v032_get_image()
{
    mt9v032_image_flag = IMG_START;                   //开始采集图像
    PORTC_FLAG_CLR(C18);                        //写1清中断标志位(必须的，不然回导致一开中断就马上触发中断)
    enable_irq(PORTC_IRQn);                         //允许PTA的中断
    while(mt9v032_image_flag != IMG_FINISH)           //等待图像采集完毕
    {
        if(mt9v032_image_flag == IMG_FAIL)            //假如图像采集错误，则重新开始采集
        {
            mt9v032_image_flag = IMG_START;           //开始采集图像
            PORTC_FLAG_CLR(C18);                //写1清中断标志位(必须的，不然回导致一开中断就马上触发中断)
            enable_irq(PORTC_IRQn);                 //允许PTA的中断
